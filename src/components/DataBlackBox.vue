...
onMounted(() => {
  const container = document.getElementById('data-black-box-transform')!
  ...
  for (let i = 0; i < 3; i++) { // 0 = xy-plane; 1 = xz-plane; 2 = yz-plane
    for (let j = 0; j < 3; j++) { // 0 = lowest cube; 2 = highest cube
      for (let k = 0; k < 2; k++) { // 0 = bottom face; 1 = top face
        ...
        let filter: any
        filter = document.getElementById(isOuterFace ? 'data-black-box-outer-face' : 'data-black-box-inner-face')
        filter = filter.insertAdjacentElement('beforebegin', filter.cloneNode(true))
        filter.id = 'data-black-box-face-' + i + j + k
        if (isOuterFace) {
...
<style>
#data-black-box-container {
  background: 66px 73px / 174px 174px url("blackbox.png"), linear-gradient(skyblue, khaki);
  width: 300px;
  height: 300px;
  margin: auto;
  display: flex;
...
#data-black-box-filter {
  width: 2em;
  height: 2em;
  margin: auto;
  display: flex;
  filter: url("#data-black-box-post-processing");
}

#data-black-box-transform {
  width: 1em;
  height: 1em;
  margin: auto;
